...
class Client:
    ...
    def load_page(self, page):
        url = f'https://sport-hunter.com.ua/{MANUFACTURER}/?page={page}&bfilter=m0:12;'
    ...
    def parse_page(self, html):
        ...
        cards = soup.select('.product-card.products-grid__item')
        if not cards:
            return False

        for card in cards:
            # Извлекаем основную информацию о товаре
            goods_name_elem = card.select_one('.product-card__title')
            goods_name = goods_name_elem.text.strip()
            link_elem = goods_name_elem.select_one('a')
            url = link_elem['href']
            price_new_elem = card.select_one('.product-card__price .newprice')
